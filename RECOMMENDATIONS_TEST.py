...
def launch_recommendation_page():
    ...
    # ---------- Header ----------
    tk.Label(root, text="SmartStock: Candlestick-Based Recommendations", font=("Helvetica", 16, "bold")).pack(pady=10)
    ...
    current_filter = tk.StringVar(value="")  # Track current filter
    ...
    btn_buy = tk.Button(button_frame, text="Buy", width=12, bg="#d4fcd4", command=lambda: filter_action("Buy"))
    btn_sell = tk.Button(button_frame, text="Sell", width=12, bg="#fcd4d4", command=lambda: filter_action("Sell"))
    btn_watch = tk.Button(button_frame, text="Watch", width=12, bg="#fcfcd4", command=lambda: filter_action("Watch"))
    ...
    tk.Button(utility_frame, text="Clear Table", command=refresh_data).grid(row=0, column=0, padx=5, sticky='ew')
    ...
    tk.Button(utility_frame, text="Export to CSV", command=export_csv).grid(row=0, column=1, padx=5, sticky='ew')
    ...
    def plot_candlestick(ticker, timeframe="1d", period="1mo"):
        ...
        mpf.plot(data, type='candle', style='charles', title=f"{ticker} Candlestick Chart", volume=True, figratio=(16, 8),figscale=1.5)
    ...
    tk.Button(utility_frame, text="View Chart", command=view_chart).grid(row=0, column=2, padx=5, sticky='ew')
